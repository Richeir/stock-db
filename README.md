# BaoStock 调研 Demo

[BaoStock](http://baostock.com/) 是一个免费、开源、Python 证券数据接口库，提供 A 股历史 K 线、证券基本信息、指数成分股、财务报表、宏观利率等数据。本仓库调研并沉淀 BaoStock 的用法，包含一个可运行的 demo、完整的 API 参考，以及一套配套的 SQLite 存储设计。

## 文档导航

| 文档 | 内容 |
|------|------|
| **README.md**（本文件） | 上手说明、核心知识点（返回值结构 / 实践注意） |
| **API.md** | 完整 API 目录、函数签名、各接口真实返回值示例 |
| **DB_DESIGN.md** | SQLite 存储设计：表结构、复权策略、写入流程 |

---

## 1. 安装与登录

项目约定使用虚拟环境 `.venv`，推荐通过 `pyproject.toml` 安装（含 baostock 依赖与 `baostock-demo` 命令入口）：

```bash
python3 -m venv .venv
.venv/bin/pip install -e .
```

> 说明：`requirements.txt` 内容为 `-e .`，即直接安装本项目，因此 `pip install -r requirements.txt` 效果等价。也可只 `pip install baostock` 后直接运行 `demo.py`。

安装后除 `python demo.py` 外，还可直接运行命令 `baostock-demo`。

所有数据查询前必须调用 `login()`，结束使用后调用 `logout()`。默认是匿名登录：

```python
import baostock as bs

lg = bs.login()        # 默认参数 user_id='anonymous', password='123456'
print(lg.error_code)   # '0' 表示成功
print(lg.error_msg)    # 'success'
# ... 查询数据 ...
bs.logout()
```

---

## 2. 返回值结构（核心知识点）

BaoStock 的返回对象**统一**用下面这套属性，是所有 `query_*` 函数与 `login()/logout()` 的通用结构：

| 属性 | 类型 | 含义 |
|------|------|------|
| `error_code` | `str` | `'0'` 表示成功，非 `'0'` 表示失败 |
| `error_msg` | `str` | 错误信息描述，成功时为 `'success'` |
| `fields` | `list[str]` | 查询结果各列的字段名（**仅 query 类返回**） |
| `data` | `list[list[str]]` | 查询结果的行数据，**每行是一个由字符串组成的列表** |
| `rows` | `int` | 总行数（**仅 query 类返回**，属性存在但 `dir()` 不可见） |

> **注意点：** 所有数值在 `data` 里都是**字符串**（如 `'6.6300'`、`'22066700'`），需要自行转换类型。

---

## 3. 实践注意事项

1. **必须登录**：未 `login()` 就查询会失败。
2. **依赖网络**：数据来自 baostock 服务器，偶尔连接失败，重试即可。
3. **返回值为字符串**：数值字段需手动 `float()` / `int()` 转换。
4. **成功判断看 `error_code`**：`error_code == '0'` 才算成功，`rows == 0` 只是"该条件下无数据"，并非错误。
5. **时间格式**：日期用 `YYYY-MM-DD` 字符串；财务接口用 `year` + `quarter`。
6. **股票代码格式**：带交易所前缀，如 `sh.600000`（沪）、`sz.000001`（深）。
7. **`rows` 是隐藏属性**：通过 `rs.rows` 可访问，但不在 `dir(rs)` 中列出。

---

## 4. 复权策略与数据范围

本项目的**复权策略**（每张 K 线表同时保存前复权 `'2'` 与不复权原始价 `'3'` 两档，并辅以复权因子表，规避前复权历史价随新除权漂移的问题），以及**股票 / ETF / 指数各频率的数据范围**，详见 [DB_DESIGN.md](DB_DESIGN.md) 第 2 节「数据范围与频率说明」和第 7.1 节「复权因子 `adjust_factor`」。

---

## 5. 数据更新时段

BaoStock 平台各数据的每日更新时段如下（供入库调度参考）：

| 时间 | 入库数据 |
|------|----------|
| 当前交易日 17:30 | 完成日 K 线数据入库 |
| 当前交易日 18:00 | 完成复权因子数据入库 |
| 当前交易日 20:00 | 完成分钟 K 线数据入库 |
| 第二自然日 1:30 | 完成前交易日"其它财务报告数据"入库 |
| 周六 17:30 | 完成周 K 线数据入库 |
| 每月 1 号 17:30 | 完成上月月 K 线数据入库 |

**每周数据更新时间：**

- 每周一下午，完成上证 50 成份股、沪深 300 成份股、中证 500 成份股信息数据入库。

---

## 6. 快速开始

```bash
# 创建虚拟环境并安装
python3 -m venv .venv
.venv/bin/pip install -e .

# 运行 demo（真实联网取数，演示历史 K 线、ETF 日 K、证券信息、
# 指数成分、财务、宏观利率、交易日历等常用 API）
.venv/bin/python demo.py
```

---

## 7. 数据入库（DB_DESIGN.md）

把 BaoStock 数据落库为 SQLite 的**存储设计**详见 [DB_DESIGN.md](DB_DESIGN.md)：包含 11 张表（基础信息、股票/ETF 日周月 K 线、技术面分析、复权因子）、复权策略（前复权为主 + 保留不复权原始价）、写入流程与各频率数据范围。本仓库聚焦 BaoStock API 的调研与测试，落库实现交由下游项目按该设计文档落地。

离线测试：`pip install -e ".[dev]"` 后运行 `.venv/bin/python -m pytest`。测试位于 `tests/test_demo.py`，通过 mock `baostock` 验证 `demo.py` 的 API 调用流程与输出，不依赖网络。
