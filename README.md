...
- **不复权**：真实成交价，除权日有跳空缺口，最真实。
- **前复权**：以最新价倒推历史，价格连续，画图 / 趋势分析用（行情软件默认）。
- **后复权**：以早期价为基准上调后续价，常用于长期收益统计。

**本项目需求**：K 线存储采用“前复权为主 + 留一份不复权原始价”，可结合复权因子重算，规避前复权历史价随新除权漂移的问题。详见 [DB_DESIGN.md](DB_DESIGN.md)。
